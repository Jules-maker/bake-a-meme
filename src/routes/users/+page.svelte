<script>
  ...
  import ChartUsers from "../../lib/components/chartUsers/chartUsers.svelte";
  ...
  const idToken =
    typeof localStorage !== "undefined"
      ...
      : null;
  let users = [];
  let searchTerm = ""; // Nouvelle propriété pour le terme de recherche
  ...
  async function deleteUser(userLogin) {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/admin/users/${userLogin}`,
        {
          method: "DELETE",
          headers: {
            Accept: "*/*",
      ...
      if (response.ok) {
        ...
        console.error(
          `DELETE request for user ${userLogin} failed:`,
          response.status,
          response.statusText
        );
      ...
      >
          ...
          <div class="w-full md:w-1/2">
            <form class="flex items-center">
              ...
              <div class="relative w-full">
                ...
                <!-- Lier la valeur du terme de recherche -->
                <input
                  type="text"
                  id="simple-search"
        ...
        <div class="overflow-x-auto">
          ...
          >
            ...
            <tbody>
              {#each users.filter((user) => user.login
                  .toLowerCase()
                  .includes(searchTerm.toLowerCase())) as user (user.id)}
                <tr class="border-b dark:border-gray-700">
                  ...
                  <td class="px-4 py-3">
                    <button
                      on:click={() => deleteUser(user.login)}
                      class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100"
                      type="button"
                    >
                      <svg
...
{:else}
<p class="my-40 mx-auto font-bold text-2xl text-center">Vous n'êtes pas connecté, vous devez avoir le rôle Admin pour y acceder</p>
{/if}
<Footer />
